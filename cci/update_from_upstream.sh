...
sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-key 23F3D4EA75716059 > /dev/null
...
RECENT_PRS=$(gh pr list --limit 800 --state merged | grep $'\t'"${GIT_GITHUB_FORK_ACCOUNT}:")
echo "${RECENT_PRS}"
echo ""

echo ""
echo "Matching commit and API based information"
echo "Delete all merged branches, which got merged via a squashed merge commit:"
for PR_ID in ${PR_IDS}
do
...
git branch -r --merged main | grep -v main | grep -v master | grep -v "action-doc-toc" | sed 's/origin\///' | xargs -r -n 1 git push --delete origin
echo ""
